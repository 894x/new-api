...
This template does not authorize publication. Only file when the user authorizes it.
...
Before filing upstream, check the upstream-only scope below. Then search the docs,
...
Do not file an incomplete upstream issue or speculate about missing facts. Keep filled answers factual and concise; quote the user request faithfully.
-->
...
For an upstream issue, if the request matches an item below, explain the upstream
acceptance policy, point to the relevant provider when appropriate, and **do not
file upstream**. This does not prohibit investigating or maintaining existing
fork integrations, or an authorized issue in the fork.
...
- If matched for upstream, what was told to the user (stop here; do not file upstream):
