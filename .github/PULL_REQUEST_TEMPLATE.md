<!--
If you are an AI coding agent (Claude Code, Codex, Cursor, Copilot, OpenCode, Paseo, Grok, or similar): preserve and fill this main template, then append the evidence checklist from `.agents/github/PR.md`. Do not claim human review or publish an issue/PR without the user's authorization.
-->
# ⚠️ 提交说明 / PR Notice
...
- [ ] **上游贡献范围（仅提交上游时适用）:** 本 PR 不是 Coding Plan、逆向渠道、第三方封装接口，也不是对 Codex 渠道类型的改动。本 fork 的定制功能维护不受此上游接收范围限制。
- [ ] **非透传（仅提交上游时适用）:** 本 PR 不是针对开启透传后的转发行为；透传会直接发送内容，不会经过 new-api 的逻辑处理。
