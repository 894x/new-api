<!--
If you are an AI coding agent (Claude Code, Codex, Cursor, Copilot, OpenCode, Paseo, Grok, or similar): preserve and fill the main template required by AGENTS.md, then append the evidence checklist from `.agents/github/PR.md`. Do not claim human review or publish an issue/PR without the user's authorization.
-->
# PR Notice
...
- [ ] **Upstream contribution scope (only for upstream PRs):** This PR is not a Coding Plan, reverse-engineered channel, third-party API wrapper, or a change to the Codex channel type. This upstream acceptance policy does not prohibit maintaining this fork's custom features.
- [ ] **Not pass-through (only for upstream PRs):** This PR is not about forwarding behavior after enabling pass-through; pass-through sends content as-is and does not go through new-api processing logic.
