...
func closeAuditDatabaseAfterTest(t *testing.T, db *gorm.DB) {
	t.Helper()
...
func newAuditTestDatabase(t *testing.T, kind, dsn string) (*gorm.DB, string) {
	...
	if kind == "sqlite" {
		...
		require.NoError(t, err)
		closeAuditDatabaseAfterTest(t, db)
		return db, path
	}
...
func verifyAuditJSONStorage(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		closeAuditDatabaseAfterTest(t, model.LOG_DB)
	}
	entries, total, err = model.GetAuditLogs(filter, 0, 20, common.RoleRootUser)
	require.NoError(t, err)
	require.EqualValues(t, 1, total)
			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					for i := 0; i < 2; i++ {
						...
						closeAuditDatabaseAfterTest(t, model.DB)
						require.NoError(t, model.InitLogDB())
						closeAuditDatabaseAfterTest(t, model.LOG_DB)
					}
					if !upgrade {
