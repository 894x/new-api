...
type advancedCustomBalanceResponseSummary struct {
	ResponseType string `json:"response_type"`
	FieldCount   int    `json:"field_count,omitempty"`
	ItemCount    int    `json:"item_count,omitempty"`
}

// summarizeAdvancedCustomBalanceResponse exposes only the JSON shape. Unknown
// upstream response values may contain credentials or account information.
func summarizeAdvancedCustomBalanceResponse(response json.RawMessage) (string, error) {
	summary := advancedCustomBalanceResponseSummary{ResponseType: common.GetJsonType(response)}
	switch summary.ResponseType {
	case "object":
		var fields map[string]json.RawMessage
		if err := common.Unmarshal(response, &fields); err != nil {
			return "", err
		}
		summary.FieldCount = len(fields)
	case "array":
		var items []json.RawMessage
		if err := common.Unmarshal(response, &items); err != nil {
			return "", err
		}
...
func fetchAdvancedCustomBalance(channel *model.Channel) (channelBalanceResult, error) {
	...
	formatted, err := summarizeAdvancedCustomBalanceResponse(validated)
	if err != nil {
		return channelBalanceResult{}, fmt.Errorf("invalid balance JSON response: %w", err)
	}
	return channelBalanceResult{ResponseSummary: formatted}, nil
}
...
func UpdateChannelBalance(c *gin.Context) {
	...
	if result.ResponseSummary == "" {
		...
		response["response_summary"] = result.ResponseSummary
	}
...
func updateAllChannelsBalance() error {
	...
	for _, channel := range channels {
		...
		if err != nil {
			continue
		} else if result.ResponseSummary == "" {
