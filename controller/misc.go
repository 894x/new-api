...
func GetHomePageContent(c *gin.Context) {
	...
	email := common.OptionMap["BusinessContactEmail"]
	qrCode := common.OptionMap["BusinessContactQRCode"]
	common.OptionMapRWMutex.RUnlock()
	// Length prefixes avoid ambiguous joins; all public fields invalidate the
	// cached response without depending on JSON key order or encoder escaping.
	validator := fmt.Sprintf("home:v1:%d:%s%d:%s%d:%s%d:%s", len(content), content, len(template), template, len(email), email, len(qrCode), qrCode)
	serveRevalidatedJSONEnvelope(c, gin.H{
		"success":                  true,
		"message":                  "",
		"data":                     content,
		"template":                 template,
		"business_contact_email":   email,
		"business_contact_qr_code": qrCode,
	}, validator)
}
