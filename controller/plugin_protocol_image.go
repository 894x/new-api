...
func respondTaskPluginImageError(c *gin.Context, taskErr *dto.TaskError) {
	...
	clientError := service.TaskErrorForClient(c, &dto.TaskError{
		StatusCode: status, Code: code, Message: message,
	})
	errorType := "new_api_error"
	if status < http.StatusInternalServerError && !service.ShouldHideErrorDetails(c) {
