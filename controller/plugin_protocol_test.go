...
func TestImageProtocolErrorsPreservePrivacyAndRequestIdentity(t *testing.T) {
	...
	require.NoError(t, operation_setting.UpdateErrorResponseReplacementRules([]operation_setting.ErrorResponseReplacementRule{{
		StatusCode: http.StatusBadRequest, Match: "vendor-secret", Replacement: "public-reason",
	}}))
	t.Cleanup(func() {
	...
	for _, tc := range []struct {
		...
		{"private user", common.RoleCommonUser, true, "request_failed", "public-reason"},
		{"administrator", common.RoleAdminUser, true, "InvalidParameter", "vendor-secret"},
		{"replacement without hiding", common.RoleCommonUser, false, "InvalidParameter", "public-reason"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			operation_setting.UpdateHideErrorDetails(tc.hide)
			c, recorder := newImageProtocolTestContext("")
			c.Set("role", tc.role)
			c.Set(common.RequestIdKey, "local-image-request")
			taskErr := &dto.TaskError{StatusCode: http.StatusBadRequest, Code: "fetch_failed", Message: `{"code":"InvalidParameter","message":"vendor-secret"}`}
			...
			assert.Contains(t, response.Error.Message, "local-image-request")
			if tc.role == common.RoleCommonUser {
				assert.NotContains(t, recorder.Body.String(), "vendor-secret")
			}
			assert.Equal(t, "fetch_failed", taskErr.Code)
			assert.Contains(t, taskErr.Message, "vendor-secret")
		})
	}
}
