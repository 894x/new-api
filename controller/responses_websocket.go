...
var responsesWSRequestEngine = sync.OnceValue(func() *gin.Engine {
	...
	engine.POST("/v1/responses", func(c *gin.Context) {
		...
		c.Next()
	}, middleware.BodyStorageCleanup(), middleware.TokenAuth(), middleware.ModelRequestRateLimit(), middleware.AssetLibraryRouting(), func(c *gin.Context) {
		state := c.Request.Context().Value(responsesWSRequestContextKey{}).(*responsesWSRequestState)
		state.apiError = state.handle(c)
		if state.apiError != nil {
			status := state.apiError.StatusCode
			publicError := service.OpenAIErrorForClient(c, state.apiError)
			state.apiError = types.WithOpenAIError(publicError, status, types.ErrOptionWithSkipRetry(), types.ErrOptionWithNoRecordErrorLog())
...
func ResponsesWebSocket(c *gin.Context) {
	...
	if apiError := relay.ResponsesWebSocketHelper(c, ws, runner); apiError != nil {
		...
		helper.WssError(c, ws, service.OpenAIErrorForClient(c, apiError))
	}
}
