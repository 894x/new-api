...
func TestResponsesWSRequestRunnerRefreshesBillingContextAndCleansBody(t *testing.T) {
	...
	require.Nil(t, runner(request, "ws-second", func(c *gin.Context) *types.NewAPIError {
		...
		require.NoError(t, err)
		assert.NotSame(t, storage, currentStorage)
		currentBody, err := currentStorage.Bytes()
		require.NoError(t, err)
		assert.JSONEq(t, `{"input":"second"}`, string(currentBody))
		return nil
	}))
}
...
func TestResponsesWSRequestRunnerRejectsRevokedCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "before", func(*gin.Context) *types.NewAPIError { return nil }))
			tc.update(t, user, token)
			called := false
			apiError := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "after", func(*gin.Context) *types.NewAPIError {
				called = true
				return nil
			})
...
func TestResponsesWSRequestRunnerUsesExistingMemoryRateLimit(t *testing.T) {
	...
	require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "first", handle))
	apiError := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "limited", handle)
	require.NotNil(t, apiError)
...
func TestResponsesWSRequestRunnerSharesRedisSuccessLimitWithHTTP(t *testing.T) {
	...
	failure := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "failed", func(*gin.Context) *types.NewAPIError { return failed })
	require.NotNil(t, failure)
	assert.Equal(t, http.StatusBadRequest, failure.StatusCode)
	require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{}`)), "successful", func(*gin.Context) *types.NewAPIError { return nil }))
	engine := gin.New()
	called := false
...
func TestResponsesWebSocketPreservesModelAdmissionPerCreate(t *testing.T) {
	for _, mode := range []string{"model RPM", "model TPM", "channel RPM"} {
		t.Run(mode, func(t *testing.T) {
			...
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
				for {
					...
					if !assert.NoError(t, common.Unmarshal(payload, &request)) {
						return
					}
					assert.Equal(t, "gpt-4o", request["model"])
					if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.completed","response":{"id":"first","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`))) {
						return
					}
				}
			})
			...
			case "model RPM":
				setting.ModelRequestRateLimitEnabled = true
				require.NoError(t, setting.UpdateModelRequestRateLimitGroupByJSONString(`{"default":{"limits":[0,1000,0],"models":{"ws-billing":{"rpm":1}}}}`))
			case "model TPM":
				setting.ModelRequestRateLimitEnabled = true
				require.NoError(t, setting.UpdateModelRequestRateLimitGroupByJSONString(`{"default":{"limits":[0,1000,0],"models":{"ws-billing":{"tpm":1000}}}}`))
			case "channel RPM":
				rpm := int64(1)
				require.NoError(t, model.DB.Create(&model.ChannelModelOverride{ChannelId: fixture.channel.Id, Model: "ws-billing", RPM: &rpm}).Error)
				require.NoError(t, model.DB.Model(&model.Ability{}).Where("channel_id = ?", fixture.channel.Id).Update("rpm", rpm).Error)
			}
			// The nested request must be normalized before the ordinary limiter
			// reads its model, and the locked connection must recheck admission.
			message := []byte(`{"type":"response.create","response":{"model":"ws-billing","input":"hello"}}`)
			require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, message))
			...
			assert.Equal(t, float64(http.StatusTooManyRequests), rejected["status"])
			require.Eventually(t, func() bool {
				var token model.Token
				var user model.User
				return model.DB.First(&token, fixture.token.Id).Error == nil &&
					model.DB.First(&user, fixture.user.Id).Error == nil &&
					token.RemainQuota == 2000 && user.Quota == 99000
			}, 5*time.Second, 10*time.Millisecond, "local admission rejection must refund before the fixture closes")
			fixture.closeAndWait(t)
			assert.Equal(t, int32(1), dispatched.Load(), "denied creates must not reach the provider")
			assertResponsesWSAccounting(t, fixture, []int{1000})
		})
	}
}

func TestResponsesWebSocketRedactsFailureWithoutLosingUsage(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("input", p * 2)`, func(ws *websocket.Conn, _ *http.Request) {
		...
		if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.failed","stream_id":"planner","response":{"id":"failed","status":"failed","error":{"code":"server_error","message":"secret-provider-token"},"usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`))) {
			return
		}
		_, _, _ = ws.ReadMessage()
	})
	operation_setting.UpdateHideErrorDetails(true)
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","stream_id":"planner","model":"ws-billing","input":"hello"}`)))
	event := readResponsesWSTestEvent(t, fixture.client)
	assert.Equal(t, "response.failed", event["type"])
	assert.Equal(t, "planner", event["stream_id"])
	payload, err := common.Marshal(event)
	require.NoError(t, err)
	assert.NotContains(t, string(payload), "secret-provider-token")
	assert.Contains(t, string(payload), "responses-ws-billing-ws-0")
	fixture.closeAndWait(t)
	assertResponsesWSAccounting(t, fixture, []int{1000})
}
...
func TestResponsesWebSocketInitialUpstreamRejectionRefundsReservation(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Input-only pre-consume does not reserve an output-priced expression.
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
				if _, _, err := ws.ReadMessage(); !assert.NoError(t, err) {
					return
				}
			...
			assert.Contains(t, rejectionError["message"], tc.wantMessage)
