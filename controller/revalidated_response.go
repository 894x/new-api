...
func serveRevalidatedJSON(c *gin.Context, content string) {
	serveRevalidatedJSONEnvelope(c, publicContentResponse{
		Success: true,
		Message: "",
		Data:    content,
	}, content)
...
// envelope may include fork-specific public fields. Its validator must cover
// every field and stay independent of the selected JSON encoder.
func serveRevalidatedJSONEnvelope(c *gin.Context, envelope any, validatorContent string) {
	...
	etag := common.ETagFor(etagVersionPublicContent, validatorContent)
