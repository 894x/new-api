...
func TestManageUserQuotaOverrideUsesVersionedAbsoluteMutation(t *testing.T) {
	db := setupManageUserTestDB(t)
	user := model.User{
		Username: "managed-quota-override-user", Password: "password", Role: common.RoleCommonUser,
		Status: common.UserStatusEnabled, Group: "default", AuthVersion: 1, Quota: 100,
	}
	require.NoError(t, db.Create(&user).Error)

	recorder := performManageUserRequest(t, fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":"override","value":42}`, user.Id))
	assert.Equal(t, http.StatusOK, recorder.Code)
	assert.Contains(t, recorder.Body.String(), `"success":true`)
	...
	assert.EqualValues(t, 1, updated.QuotaVersion)
}
...
func TestManualQuotaAdjustmentPreservesInFlightReservation(t *testing.T) {
	for _, tc := range []struct {
		mode                   string
		value, after, refunded int
	}{
		{"add", 500, 1300, 1500}, {"subtract", 100, 700, 900}, {"override", 600, 600, 800},
	} {
		t.Run(tc.mode, func(t *testing.T) {
			db := setupManageUserTestDB(t)
			createQuotaTestOperator(t, db, common.RoleRootUser)
			user := model.User{Username: "reserved-quota", Quota: 1000, QuotaVersion: 7}
			require.NoError(t, db.Create(&user).Error)
			reserved, err := model.ReserveUserQuotaForBilling(user.Id, 200, true)
			require.NoError(t, err)
			require.True(t, reserved)
			response := performManageUserRequest(t, fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":%q,"value":%d}`, user.Id, tc.mode, tc.value))
			require.Contains(t, response.Body.String(), `"success":true`)
			require.NoError(t, db.First(&user, user.Id).Error)
			assert.Equal(t, tc.after, user.Quota)
			assert.EqualValues(t, 9, user.QuotaVersion)
			require.NoError(t, model.RefundUserQuotaForBilling(user.Id, 200))
			require.NoError(t, db.First(&user, user.Id).Error)
			assert.Equal(t, tc.refunded, user.Quota)
			assert.EqualValues(t, 10, user.QuotaVersion)
		})
	}
}
...
func TestManageUserQuotaCacheRejectsStaleSnapshots(t *testing.T) {
	for _, tc := range []struct {
		name, mode                          string
		before, cached, value, after        int
		failUpdate, failCache, missingCache bool
	}{
		{name: "add_discards_stale_quota", mode: "add", before: 1000, cached: 900, value: 500, after: 1500},
		{name: "subtract_discards_stale_quota", mode: "subtract", before: 1000, cached: 900, value: 500, after: 500},
		{name: "override_discards_stale_quota", mode: "override", before: 1000, cached: 900, value: 2000, after: 2000},
		...
		{name: "rollback_keeps_database_balance", mode: "subtract", before: 1000, cached: 900, value: 500, after: 1000, failUpdate: true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.after, user.Quota)
			if !tc.failCache {
				assert.False(t, server.Exists(quotaKey), "old quota must remain fenced out")
				current, err := model.GetUserCache(user.Id)
				require.NoError(t, err)
				assert.Equal(t, tc.after, current.Quota)
				assert.Equal(t, user.QuotaVersion, current.QuotaVersion)
			}
		})
	}
}
