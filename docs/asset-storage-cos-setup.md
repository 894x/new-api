...
查询自己用量：`GET /api/asset-library/storage`，使用现有账号或 API Token 鉴权。响应 `data` 包含 `enabled`、`quota_mb`、`used_bytes`、`remaining_bytes`、`bytes_per_mb`。余额或 token 消费额度不参与此计算。
...
- 生成结果自动归档、单用户额度覆盖、付费存储和 COS 历史版本清理不在本版范围。
