...
is currently unreleased; [`v1.schema.json`](./v1.schema.json) describes manifest
shapes and [`v1.d.ts`](./v1.d.ts) describes hook signatures. The gateway's
runtime validation additionally enforces semantic constraints, routing conflicts,
usage bounds, and security policy; schema validation alone is not acceptance.
...
For successful legacy tasks without provider data, artifact hooks also receive
the private `resultUrl` fallback from persisted task state. It is empty for all
other statuses and must never be returned directly to clients; content still
passes through the host's signed artifact proxy and fetch safeguards.
...
Deployment switches and protocol observation settings are documented in
