...
Query and content contexts are not submit contexts. `buildQueryRequest` receives
the upstream `taskId`, `action`, polling `requestBody`, base URL, channel settings,
and resolved authentication. `buildBatchQueryRequest` receives base URL,
channel settings and authentication, plus the task IDs as a separate argument.
Result parsers receive an empty context and the response body; they must not
expect submission fields or credentials.

Artifact hooks use a separate camelCase task shape: `taskId`, `status`, `action`,
`data`, `producerVersion`, and `resultUrl`. `resultUrl` is an internal fallback
