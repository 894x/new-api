...
func PrepareTaskPluginRoute() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		case string(pluginruntime.RouteTypeSubmit):
			...
			// Native routes may carry a channel-configured public alias. Keep it
			// unchanged through decoding and verify its mapped provider model in
			// the adaptor before pricing or upstream dispatch. A decoder must not
			// manufacture an unowned model or capture another plugin's model.
			bodyValue, _ := bodyObject["value"].(map[string]any)
			claimedModel, _ := bodyValue["model"].(string)
			alias := !owned && modelName == claimedModel
			if alias && pinned.Generation != nil {
				if owner, found := pinned.Generation.GetByModel(modelName); found && owner != pinned.Plugin {
					alias = false
				}
			}
			if !owned && !alias {
...
// candidate parsers pinned before distribution. Only providers whose decoder
// accepts the request remain eligible for shared-model channel selection.
func PrepareTaskPluginEndpoint() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(pinned.Candidates) > 1 {
			...
			var firstResolved any
			for _, candidate := range pinned.Candidates {
				value, err := resolvedValue, callErr
				if candidate.Plugin != pinned.Plugin {
					value, err = candidate.Plugin.Engine.CallPathWithAdmissionTimeout(
						context.WithoutCancel(c.Request.Context()), pluginruntime.DefaultCallTimeout,
						"protocols", []string{candidate.Protocol, "decodeRequest"}, protocolContext.JSValue(),
					)
				}
				result, valid := value.(map[string]any)
				if err != nil || !valid || result["kind"] != string(pluginruntime.RouteTypeSubmit) || result["model"] != pinned.Model {
					continue
				}
				if _, forbidden := result["renderer"]; forbidden {
					continue
				}
				if action, present := result["action"]; present {
					if _, valid := action.(string); !valid {
						continue
					}
				}
				if len(accepted) == 0 {
			...
			if len(accepted) > 0 {
				...
				resolvedValue, callErr = firstResolved, nil
			} else if callErr == nil {
				callErr = fmt.Errorf("Invalid task protocol request")
			}
		}
		if callErr != nil {
			logger.LogWarn(
...
func renderTaskPluginQuery(
	...
	for _, taskID := range taskIDs {
		...
		view, viewErr := service.BuildTaskPluginViewForClient(c, task)
		if viewErr != nil {
			abortTaskPluginRouteError(c, http.StatusInternalServerError)
			return
		}
...
func RespondTaskPluginError(c *gin.Context, taskErr *dto.TaskError) bool {
	...
	sanitized = service.TaskPluginErrorForClient(c, sanitized, taskErr.Message)
...
func abortTaskPluginRouteErrorDetail(c *gin.Context, status int, detail string) {
	...
	taskErr = service.TaskPluginErrorForClient(c, taskErr, detail)
