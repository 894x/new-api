...
// Diagnostic contract tests use an explicit administrator. Customer redaction
// is exercised separately through both native and shared error paths.
func taskPluginDiagnosticViewer(c *gin.Context) {
...
func TestPrepareTaskPluginEndpointRejectsModelDriftBeforeDistribution(t *testing.T) {
	...
	reachedDistribution := false
	router := gin.New()
	router.Use(taskPluginDiagnosticViewer)
	router.POST(
		"/v1/responses",
		PinTaskPluginEndpoint(),
		PrepareTaskPluginEndpoint(),
...
`)
	...
	// Native renderers receive a structured requestId, so its message has no suffix.
	publicText := strings.TrimSuffix(service.PublicErrorMessage(""), " (request id: )")
	...
	}`, publicText), recorder.Body.String())
...
func TestTaskPluginErrorFallbackIsSanitized(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			assert.Equal(t, http.StatusBadGateway, recorder.Code)
			assert.JSONEq(t, fmt.Sprintf(`{"code":"request_failed","message":%q,"data":null}`, service.PublicErrorMessage("fallback-req")), recorder.Body.String())
			assert.NotContains(t, recorder.Body.String(), "upstream.invalid")
...
func TestPrepareTaskPluginEndpointAdmitsOnlyCompatibleSeedanceProviders(t *testing.T) {
	...
	for _, key := range []string{"doubao", "seedance-sls"} {
		...
		require.NoError(t, err)
		_, err = jsplugin.DefaultRegistry.RegisterFactory(source, jsplugin.Options{Key: key})
		require.NoError(t, err)
	}
	for _, tc := range []struct {
		...
		{"automatic SLS only", -1, []string{"seedance-sls"}, http.StatusNoContent},
		{"standard both providers", 5, []string{"doubao", "seedance-sls"}, http.StatusNoContent},
		{"invalid neither provider", -2, nil, http.StatusBadRequest},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var accepted []string
			router := gin.New()
			router.POST("/v1/videos", PinTaskPluginEndpoint(), PrepareTaskPluginEndpoint(), func(c *gin.Context) {
				pin := c.MustGet(jsplugin.ContextKeyPinnedEndpoint).(jsplugin.PinnedEndpoint)
				for _, candidate := range pin.Candidates {
					accepted = append(accepted, candidate.Plugin.Meta.Key)
				}
				assert.Equal(t, tc.expected[0], c.GetString("expected_task_plugin_key"))
				filters := service.GetChannelConstraints(c).Filters
				for _, provider := range []struct {
					key         string
					channelType int
				}{{"doubao", constant.ChannelTypeDoubaoVideo}, {"seedance-sls", constant.ChannelTypeSeedanceSLS}, {"unrelated", constant.ChannelTypeMiniMax}} {
					want := false
					...
					for _, channelType := range []int{provider.channelType, constant.ChannelTypeTaskPlugin} {
						settings := fmt.Sprintf(`{"task_plugin_key":%q}`, provider.key)
						ch := &model.Channel{Type: channelType, Setting: &settings}
						allowed, _ := model.ChannelSatisfiesFilters(ch, pin.Model, filters)
						assert.Equal(t, want, allowed, "provider=%s channel=%d", provider.key, channelType)
					}
				}
				c.Status(http.StatusNoContent)
			})
			request := httptest.NewRequest(http.MethodPost, "/v1/videos", strings.NewReader(fmt.Sprintf(`{"model":"doubao-seedance-2-5-260628","content":[{"type":"text","text":"a fox"}],"duration":%d}`, tc.duration)))
