...
// WithAuditActor freezes the authenticated actor before handlers can select a
// different resource owner. Background work without an actor remains root-only.
func WithAuditActor(ctx context.Context, userID, role int, username string) context.Context {
	return context.WithValue(ctx, auditActorContextKey{}, AuditActor{UserID: userID, Role: role, Username: username})
}

func AuditActorFromContext(ctx context.Context) AuditActor {
	actor, _ := ctx.Value(auditActorContextKey{}).(AuditActor)
	return actor
}
