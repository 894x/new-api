...
// ShouldRecordOperationAuditLog keeps read-only asset diagnostics in server
// logs, while retaining mutation events in the unified audit store.
func ShouldRecordOperationAuditLog(action string, params map[string]interface{}) bool {
	if action != "asset_library.request" {
		return true
	}
	switch params["action"] {
	case "CreateAsset", "AutoImport", "ReplicateAsset", "SyncAssetReplicas", "SyncAssetGroupReplicas", "SyncAssetLibraryChannel":
		return true
	default:
		return false
	}
...
// RecordOperationAuditLog 将管理、安全和素材事件写入独立审计表。
...
func RecordOperationAuditLog(logUserId, actorRole int, content string, ip string, action string, params map[string]any, adminInfo *AuditAdminInfo, auditInfo *AuditRequestInfo, request ...*gin.Context) {
	if !ShouldRecordOperationAuditLog(action, params) {
		return
	}
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	if _, exists := params.Other.public["quota_per_unit"]; !exists {
		params.Other.SetPublic("quota_per_unit", common.QuotaPerUnit)
	}
...
func GetLogsForExport(query LogExportQuery, limit int) (logs []*Log, total int64, err error) {
	...
	if err = tx.Order(order).Limit(limit + 1).Find(&logs).Error; err != nil {
		return nil, 0, err
	}
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
...
func GetUsageLogsByRequestIDsForExport(requestIDs []string, limit int) (logs []*Log, truncated bool, err error) {
	const requestIDBatchSize = 500
	for start := 0; start < len(requestIDs); start += requestIDBatchSize {
		end := min(start+requestIDBatchSize, len(requestIDs))
		var batch []*Log
		remaining := limit + 1 - len(logs)
		if remaining <= 0 {
			return logs, true, nil
		}
		err = LOG_DB.Where("logs.type IN ?", []int{LogTypeConsume, LogTypeError}).
			Where("logs.request_id IN ?", requestIDs[start:end]).
			Limit(remaining).
			Find(&batch).Error
		if err != nil {
