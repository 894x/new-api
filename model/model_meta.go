...
func (mi *Model) Update() error {
	...
	return metadataTransaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Model{}).Where("id = ?", mi.Id).
			Select("model_name", "description", "icon", "tags", "vendor_id", "endpoints", "status", "doc_enabled", "sync_official", "name_rule", "updated_time").Updates(mi).Error
	})
}
...
func DeleteModelMetadata(ids []int, removeFromChannels, removePricing bool) (ModelDeleteResult, error) {
	...
	deleteRecords := func(tx *gorm.DB) error {
		...
		if removeFromChannels {
			...
			if err := lockForUpdate(tx).Select("id", "models", "status", "group", "priority", "weight", "rpm", "tpm", "tag").Order("id").Find(&channels).Error; err != nil {
				return err
			}
		...
		if err := tx.Where("model_id IN ?", modelIDs).Delete(&ModelDocumentVariant{}).Error; err != nil {
			return err
		}
		if err := tx.Where("model_id IN ?", modelIDs).Delete(&ModelDocument{}).Error; err != nil {
			return err
		}
