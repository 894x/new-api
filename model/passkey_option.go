...
func UpdatePasskeyDomainOptions(values map[string]string, preview bool, confirmation string) (*PasskeyDomainChange, error) {
	...
	if updatesGroupPricing {
		...
		currentGroups, currentTiered := currentGroupPricingOptionValues()
		if !hasGroupRatios {
			groupRatios = currentGroups
		}
		if !hasModelTieredRatios {
			modelTieredRatios = currentTiered
		}
	...
	for key, value := range values {
		if updatesGroupPricing && (key == "GroupRatio" || key == ratio_setting.ModelTieredRatiosOptionKey) {
			continue
		}
