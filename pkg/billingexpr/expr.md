...
Task request probes are frozen before asset rewriting and channel overrides.
Native protocol wrappers are unwrapped so `param()` sees the client's native
JSON. Static paths retain only their selected JSON values and static headers
retain only their referenced values, including probes in initially unvisited
branches. Dynamic paths/headers require the corresponding full source to remain
in private billing state. Administrators should prefer static probes to avoid
retaining unrelated prompt/media or credentials. Referenced time functions use
the submission timestamp, not the later polling time. These private inputs are
never emitted in task/usage logs; logs carry only the evaluated `request_rules`
trace. Historical snapshots without request probes retain legacy evaluation.
...
An active monthly group/model discount still creates a billing session when the input-only estimate is zero, including after an auto-group retry from a free group. A zero reservation is not proof of a free final request: output usage can produce a nonzero settlement and must advance the monthly ledger exactly once.
