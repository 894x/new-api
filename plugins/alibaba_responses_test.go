...
func TestAlibabaImageResults(t *testing.T) {
	...
	t.Run("failed and malformed synchronous results do not complete a task", func(t *testing.T) {
		for _, body := range []map[string]any{
			...
			value, err := plugin.Engine.Call(t.Context(), "parseSubmitResponse", map[string]any{"upstreamModel": "wan2.6-t2i", "publicTaskId": "task_public", "requestBody": map[string]any{"model": "wan2.6-t2i", "prompt": "a cat", "metadata": map[string]any{"upstream_mode": "sync"}}}, map[string]any{"body": body})
			if body["code"] != nil {
				require.NoError(t, err)
				parsed := alibabaObject(t, value)
