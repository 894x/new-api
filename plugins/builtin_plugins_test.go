...
func TestBuiltInTaskPluginResponsesAndUsageContracts(t *testing.T) {
	expectedKeys := []string{"alibaba", "doubao", "google", "hailuo", "jimeng", "kling", "seedance-sls", "sora", "sunoapi", "tencent-tokenhub", "vertex-ai", "vidu"}
	generation := jsplugin.DefaultRegistry.Generation()
	require.NotNil(t, generation)
	...
	for _, key := range expectedKeys {
		t.Run(key, func(t *testing.T) {
			...
			if key == "seedance-sls" {
				// Preserve this custom channel's video-only contract. Sharing
				// Seedance model names does not grant Doubao's Responses support.
				assert.False(t, foundResponses)
				for _, model := range plugin.Meta.Models {
					_, claimed := registry.Generation().LookupEndpoint("POST", "/v1/responses", model)
