...
func TestHailuoH3RejectsOutOfContractRequests(t *testing.T) {
	...
	testCases := []struct {
		...
		{"adaptive ratio without media", map[string]any{"prompt": "p", "metadata": map[string]any{"ratio": "adaptive"}}, "ratio adaptive requires an image, video, or audio input"},
		...
		{"content is not an array", map[string]any{"prompt": "p", "metadata": map[string]any{"content": "nope"}}, "metadata.content must be an array"},
		{
			...
			wantErr: "at most one first_frame and one last_frame image",
		},
		{
		...
		{"empty input", map[string]any{"prompt": "  "}, "requires a non-empty text item"},
	}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
...
func TestHailuoParseSubmitResponse(t *testing.T) {
	...
	t.Run("H3 rejection reports the envelope message", func(t *testing.T) {
		...
		assert.Equal(t, map[string]any{"code": "2013", "message": "hailuo api error: invalid params", "httpStatus": float64(400)}, parsed["error"])
	})
	t.Run("H3 rejection reports the v2 error message", func(t *testing.T) {
		...
		assert.Equal(t, map[string]any{"code": "bad_request_error", "message": "content requires text", "httpStatus": float64(400)}, parsed["error"])
	})
	...
	t.Run("legacy error", func(t *testing.T) {
		...
		assert.Equal(t, map[string]any{"code": "1026", "message": "hailuo api error: sensitive", "httpStatus": float64(400)}, parsed["error"])
	})
}
