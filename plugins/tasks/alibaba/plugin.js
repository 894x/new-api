...
function isWan3(model) {
  return modelKey(model) === "wan3.0-video" || modelKey(model) === "wan3.0-video-prime";
}
...
function convert(ctx) {
  ...
  return Object.assign({}, req.nativeRequest || {}, { model: upstreamModel, input: input, parameters: parameters });
}
...
export function buildQueryRequest(ctx) {
  return { url: apiRoot(ctx) + "/api/v1/tasks/" + encodeURIComponent(ctx.taskId), method: "GET", headers: { Authorization: "Bearer " + ctx.apiKey } };
}
...
export function parseTaskResult(ctx, body) {
  ...
  if (output.task_status === "SUCCEEDED") {
    ...
    return { status: "SUCCESS", url: videoURL(body), usage: wanUsage(body.usage) };
  }
...
export function listArtifacts(task) {
  ...
  return videoURL(body) || trimmed(task.resultUrl) ? [{ key: "video", type: "video" }] : [];
}

export function buildContentRequest(ctx) {
  let url;
  if (ctx.artifactKey === "video") url = videoURL(artifactData(ctx)) || trimmed(ctx.resultUrl);
  else {
...
export const native = {
  ...
  createVideoTask: function (ctx) {
    ...
    const requestBody = {
      model: req.model,
      nativeRequest: Object.fromEntries(Object.entries(req).filter(([key]) => !["input", "parameters"].includes(key))),
