...
function upstreamPaths(ctx) {
  ...
  const mode = settings.doubao_video_api_mode || "v3";
...
export function buildSubmitRequest(ctx) {
  ...
  return {
    url: ctx.baseUrl.replace(/\/+$/, "") + upstreamPaths(ctx)[0],
    method: "POST",
...
export function buildQueryRequest(ctx) {
  return {
    url: ctx.baseUrl.replace(/\/+$/, "") + upstreamPaths(ctx)[1].replaceAll("{id}", encodeURIComponent(ctx.taskId)),
    method: "GET",
