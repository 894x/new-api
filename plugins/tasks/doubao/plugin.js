...
function upstreamPaths(ctx) {
  const settings = ctx.channelSettings || {};
  const mode = ctx.upstream && ctx.upstream.kind === "new_api" ? "v3" : settings.doubao_video_api_mode || "v3";
  if (mode === "v3") return ["/api/v3/contents/generations/tasks", "/api/v3/contents/generations/tasks/{id}"];
  if (mode === "video_generations") return ["/v1/video/generations", "/v1/video/generations/{id}"];
  if (mode !== "custom") throw new Error("unsupported doubao video api mode: " + mode);
  const paths = [trimmed(settings.doubao_video_submit_path), trimmed(settings.doubao_video_fetch_path)];
  for (const path of paths) {
    if (!path.startsWith("/") || path.startsWith("//") || path.includes("://")) throw new Error("doubao video path must be a relative path starting with /");
  }
  if (!paths[1].includes("{id}")) throw new Error("doubao video fetch path must contain {id}");
  return paths;
}
...
function validateDuration(req, metadata) {
  // Validate every alias, including overwritten values in metadata, before
  // estimating tokens or constructing the provider request.
  for (const source of [req, metadata]) {
    for (const field of ["seconds", "duration"]) {
      if (source[field] === undefined) continue;
      const value = source[field];
      const number = Number(value);
      if ((typeof value !== "number" && typeof value !== "string") || !Number.isInteger(number) || number < 1 || number > 3600)
        throw new Error(field + " must be an integer between 1 and 3600");
    }
    if (source.frames !== undefined) {
      const frames = source.frames;
      if ((typeof frames !== "number" && typeof frames !== "string") || !Number.isInteger(Number(frames)) || Number(frames) < 1 || Number(frames) > 3600 * 24)
...
function apiRoot(ctx) {
  return ctx.baseUrl.replace(/\/+$/, "") + (ctx.upstream && ctx.upstream.kind === "new_api" ? "/doubao" : "");
}
...
function videoResolution(ctx) {
  ...
  const raw = trimmed(req.resolution || metadata.resolution || req.size).toLowerCase();
...
export const native = {
  createTask: function (ctx) {
    ...
    validateDuration(body, requestMetadata(body));
    // Native content is already the provider payload. Keep its order, text
    // segments, explicit zero/false values, and future provider fields intact.
  ...
  taskStatus: function (ctx, task) {
    ...
    const output = Object.assign({}, data, {
      id: task.task_id,
      model: task.model || data.model || "",
      status: statusMap[task.status] || "queued",
      created_at: task.created_at,
      updated_at: task.updated_at,
    });
    if (task.status === "SUCCESS" && task.result_url) {
      const content = output.content && typeof output.content === "object" && !Array.isArray(output.content) ? output.content : {};
      output.content = Object.assign({}, content, { video_url: content.video_url || task.result_url });
    }
    if (task.status === "FAILURE" && task.fail_reason && !output.error) output.error = { message: task.fail_reason };
    return output;
  },
...
export function buildSubmitRequest(ctx) {
  ...
  const body = Object.assign({}, metadata, req);
  for (const field of ["metadata", "prompt", "images", "image", "input_reference", "seconds", "size"]) delete body[field];
  if (req.size && !body.resolution) body.resolution = normalizeResolution(req.size);

  // Validate against the executing model before reserving quota.
  videoResolution(ctx);
  const imageContent = [];
  const images = Array.isArray(req.images) ? req.images.slice() : [];
  for (const image of [req.image, req.input_reference]) {
    if (trimmed(image) && !images.includes(image)) images.push(image);
  }
  ...
  if (req.content !== undefined) {
    if (!Array.isArray(req.content)) throw new Error("content must be an array");
    body.content = imageContent.concat(req.content);
  } else {
    body.content = imageContent.concat(metadataContent).filter((item) => item && item.type !== "text");
    if (trimmed(req.prompt) || !body.content.length) body.content.push({ type: "text", text: req.prompt || "" });
  }
  ...
  if (req.seconds !== undefined) body.duration = Number(req.seconds);
  body.model = ctx.upstreamModel || body.model;
  return {
    url: apiRoot(ctx) + upstreamPaths(ctx)[0],
    method: "POST",
    headers: { "Content-Type": "application/json", Accept: "application/json", Authorization: "Bearer " + ctx.apiKey },
    body: body,
...
export function extractUsage(ctx) {
  ...
  if (!Number.isFinite(seconds) || seconds <= 0) {
    const frames = Number(req.frames || metadata.frames);
  ...
  if (profile.audio) facts.generate_audio = (req.generate_audio === undefined ? metadata.generate_audio : req.generate_audio) !== false;
  return facts;
}
...
export function buildQueryRequest(ctx) {
  return {
    url: apiRoot(ctx) + upstreamPaths(ctx)[1].replaceAll("{id}", encodeURIComponent(ctx.taskId)),
    method: "GET",
...
protocols.openai_video = {
  decodeRequest: function (ctx) {
    ...
    if (ctx.body.kind === "json") {
      ...
      const req = ctx.body.value;
      validateDuration(req, requestMetadata(req));
      return {
        kind: "submit",
        model: ctx.model,
