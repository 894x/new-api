...
func newResponsesStreamError(data string, responseStatus int) *types.NewAPIError {
	statusCode := helper.ResolveUpstreamStreamErrorStatus(data, responseStatus)
	var streamResponse dto.ResponsesStreamResponse
	if err := common.UnmarshalJsonStr(data, &streamResponse); err == nil && streamResponse.Response != nil {
		if oaiError := streamResponse.Response.GetOpenAIError(); oaiError != nil && oaiError.IsPresent() {
	...
	return types.NewOpenAIError(
		fmt.Errorf("upstream responses stream error: %s", common.LocalLogPreview(data)),
