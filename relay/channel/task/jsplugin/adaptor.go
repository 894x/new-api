...
type submitResponse struct {
	...
	Error     *submitError `json:"error,omitempty"`
}
...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	nativeFormat := common.GetContextKeyString(c, constant.ContextKeyTaskResponseFormat)
	if nativeFormat != "" {
		if taskErr := a.prepareNativeCompatibilityRequest(c, info, nativeFormat); taskErr != nil {
			return taskErr
		}
	}
	// Custody rewrites the request after routing, before validation. Decode the
	// stored/signed references, not the routing-time snapshot of source URLs.
	var managedBody map[string]any
	if _, managed := c.Get("managed_video_request"); managed && nativeFormat == "" {
		if err := common.UnmarshalBodyReusable(c, &managedBody); err != nil || managedBody == nil {
			return service.TaskErrorWrapperLocal(fmt.Errorf("invalid managed task request"), "invalid_request", http.StatusBadRequest)
		}
	}
	if managedBody != nil {
		if pinnedValue, exists := c.Get(pluginruntime.ContextKeyPinnedRoute); exists {
			if pinned, ok := pinnedValue.(pluginruntime.PinnedRoute); ok && pinned.Plugin == a.plugin {
				...
				if !ok || pinned.Route.Decode == "" {
					return service.TaskErrorWrapperLocal(fmt.Errorf("missing native task decoder"), "plugin_request_invalid", http.StatusBadRequest)
				}
				requestContext.Body = map[string]any{"kind": "json", "value": managedBody}
				value, err := a.plugin.Engine.CallMember(c.Request.Context(), "native", pinned.Route.Decode, requestContext.JSValue())
				resolved, valid := value.(map[string]any)
				if err != nil || !valid || resolved["kind"] != "submit" || resolved["model"] != info.OriginModelName {
					return service.TaskErrorWrapperLocal(fmt.Errorf("final native task decoder rejected the pinned model"), "plugin_request_invalid", http.StatusBadRequest)
				}
				...
				requestContext.RequestBody = managedBody
		...
		if pinned, ok := pinnedValue.(pluginruntime.PinnedEndpoint); ok && pinned.Plugin == a.plugin {
			if protocolValue, present := c.Get(pluginruntime.ContextKeyProtocolRequest); present {
				if protocolContext, valid := protocolValue.(pluginruntime.ProtocolRequestContext); valid {
					if managedBody != nil {
						protocolContext.Body = map[string]any{"kind": "json", "value": managedBody}
						c.Set(pluginruntime.ContextKeyProtocolRequest, protocolContext)
						c.Set("task_request", managedBody)
					}
	...
	if _, exists := c.Get("task_request"); !exists {
		if strings.Contains(c.GetHeader("Content-Type"), "application/json") {
			...
			if err := common.UnmarshalBodyReusable(c, &request); err != nil || request == nil {
				return service.TaskErrorWrapperLocal(fmt.Errorf("task request must be a JSON object"), "invalid_request", http.StatusBadRequest)
			}
			// Preserve provider fields for model aliases that are not claimed by
			// the shared endpoint registry. The provider decoder owns validation;
			// template/audio-driven tasks do not universally require a prompt.
			callable, err := a.plugin.Engine.HasCallablePath(c.Request.Context(), "protocols", "openai_video", "decodeRequest")
			if err != nil {
				return service.TaskErrorWrapperLocal(err, "plugin_request_invalid", http.StatusBadRequest)
			}
			if callable {
				value, err := a.plugin.Engine.CallPath(c.Request.Context(), "protocols", []string{"openai_video", "decodeRequest"}, map[string]any{
					"model": info.OriginModelName,
					"body":  map[string]any{"kind": "json", "value": request},
				})
				resolved, ok := value.(map[string]any)
				if err != nil || !ok {
					return service.TaskErrorWrapperLocal(fmt.Errorf("plugin request decoder failed: %v", err), "plugin_request_invalid", http.StatusBadRequest)
				}
		...
		} else if taskErr := relaycommon.ValidateBasicTaskRequest(c, info, "image_to_video"); taskErr != nil {
			return taskErr
		}
	}
...
// ValidateMappedRequest rebuilds the provider payload after channel model
// mapping. Neither the wire model nor model-specific validation may reuse
// the descriptor that was produced for the public alias during admission.
func (a *TaskAdaptor) ValidateMappedRequest(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	...
	if value, exists := c.Get(pluginruntime.ContextKeyPinnedRoute); exists {
		if pinned, ok := value.(pluginruntime.PinnedRoute); ok && pinned.Plugin == a.plugin &&
			!slices.Contains(a.plugin.Meta.Models, info.OriginModelName) && !slices.Contains(a.plugin.Meta.Models, info.GetUpstreamModelName()) {
			return service.TaskErrorWrapperLocal(fmt.Errorf("model %q must map to a model served by this plugin", info.OriginModelName), "plugin_model_not_owned", http.StatusBadRequest)
		}
	}
	...
	if a.plugin.Meta.Key == "seedance-sls" {
		...
		if prepared["model"] != info.GetUpstreamModelName() {
			return service.TaskErrorWrapperLocal(fmt.Errorf("use channel model mapping to change the Seedance model"), "invalid_request", http.StatusBadRequest)
		}
...
func (a *TaskAdaptor) AdjustBillingOnComplete(task *model.Task, result *relaycommon.TaskInfo) int {
	...
	if a.hasHook(context.Background(), "extractUsageOnComplete") {
		value, hookErr := a.plugin.Engine.Call(context.Background(), "extractUsageOnComplete", jsonValue(view), jsonValue(result), view.Data)
		if hookErr == nil {
			a.applyCompletionUsageFacts(result, value)
		}
	}
	bc := task.PrivateData.BillingContext
	if bc == nil || bc.PerCallBilling || bc.TieredSnapshot != nil || bc.ModelRatio <= 0 || !a.hasHook(context.Background(), "extractBillingOnComplete") {
		return 0
	}
	value, err := a.plugin.Engine.Call(context.Background(), "extractBillingOnComplete", jsonValue(view), jsonValue(result), map[string]any{"otherRatios": jsonValue(bc.OtherRatios), "upstreamModel": task.Properties.UpstreamModelName})
	if err != nil {
		a.logRejectedUsage("extractBillingOnComplete", err)
		return 0
	}
	if value == nil {
		return 0
	...
	var billing struct {
		ModelUnits     float64  `json:"modelUnits"`
		ConsumedRatios []string `json:"consumedRatios"`
	}
	if err = convert(value, &billing); err != nil || math.IsNaN(billing.ModelUnits) || math.IsInf(billing.ModelUnits, 0) || billing.ModelUnits <= 0 {
		a.logRejectedUsage("extractBillingOnComplete", fmt.Errorf("invalid model units"))
		return 0
	}
	// Plugins report model units, never currency or account balances. The host
	// retains conversion, frozen pricing, monthly discounts and saturation audit.
	tokens, clamp := common.QuotaRoundChecked(billing.ModelUnits * common.QuotaPerUnit / 2)
	if tokens <= 0 {
		return 0
	}
	result.TotalTokens, result.QuotaClamp = tokens, clamp
	for _, key := range billing.ConsumedRatios {
		if _, exists := bc.OtherRatios[key]; exists {
			bc.OtherRatios[key] = 1
		}
	}
	return 0
}
...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	if descriptor.BodyType == "multipart" {
		...
		// The host generated this boundary, so it must override any plugin
		// header and travel with the encoded multipart body to the provider.
	...
	// The host owns logical asset authorization and channel replicas. The JS
	...
	if a.plugin.Meta.Key == "alibaba" {
		if payload, ok := inlined.(map[string]any); ok {
			inlined, err = service.RewriteAssetReferences(info.UserId, info.ChannelId, payload)
			if err != nil {
				return nil, err
			}
		}
...
func (a *TaskAdaptor) ParseResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*channel.TaskSubmitResponse, *dto.TaskError) {
	...
	if err = convert(value, &parsed); err == nil && parsed.Error != nil && strings.TrimSpace(parsed.Error.Message) != "" {
		status := parsed.Error.HTTPStatus
		...
		return nil, service.TaskErrorWrapper(fmt.Errorf("%s", parsed.Error.Message), code, status)
	}
	if err != nil || strings.TrimSpace(parsed.TaskID) == "" {
		if err == nil {
	...
	if parsed.TaskData != nil {
		...
		taskData = a.SanitizeTaskData(taskData, info.PublicTaskID)
	}
	var immediate *relaycommon.TaskInfo
	if parsed.Immediate != nil {
		if err := validateTaskUsage(parsed.Immediate.Usage); err != nil {
			return nil, service.TaskErrorWrapper(err, "plugin_submit_response_invalid", http.StatusBadGateway)
		}
		immediate = &relaycommon.TaskInfo{Code: parsed.Immediate.Code, TaskID: parsed.Immediate.TaskID, Status: parsed.Immediate.Status, Progress: parsed.Immediate.Progress, Reason: parsed.Immediate.Reason, Url: parsed.Immediate.URL, RemoteUrl: parsed.Immediate.RemoteURL, Usage: parsed.Immediate.Usage, TotalTokens: positiveInt(parsed.Immediate.TotalTokens), CompletionTokens: positiveInt(parsed.Immediate.CompletionTokens)}
		if a.hasHook(c.Request.Context(), "extractUsageOnComplete") {
			facts, hookErr := a.plugin.Engine.Call(c.Request.Context(), "extractUsageOnComplete", nil, jsonValue(immediate), responseBody)
			if hookErr == nil {
				a.applyCompletionUsageFacts(immediate, facts)
			}
		}
	}
	logger.LogDebug(
...
func (a *TaskAdaptor) ParseSubmitError(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) *dto.TaskError {
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return service.TaskErrorWrapper(err, "read_response_body_failed", http.StatusBadGateway)
	}
	fallback := service.TaskErrorWrapper(fmt.Errorf("%s", body), "fail_to_fetch_task", resp.StatusCode)
	if !a.hasHook(c.Request.Context(), "parseSubmitError") {
		return fallback
	}
	...
	value, err := a.plugin.Engine.Call(c.Request.Context(), "parseSubmitError", a.submitContext(c, info), map[string]any{"statusCode": resp.StatusCode, "body": payload})
	...
	if convert(value, &parsed) != nil || strings.TrimSpace(parsed.Message) == "" {
		return fallback
	}
	if parsed.Code == "" {
		parsed.Code = "fail_to_fetch_task"
	}
	return service.TaskErrorWrapper(fmt.Errorf("%s", parsed.Message), parsed.Code, resp.StatusCode)
}
...
func (a *TaskAdaptor) ParseBatchResult(tasks []*model.Task, resp *http.Response, body []byte) (map[string]*service.BatchTaskResult, error) {
	...
	for _, item := range parsed {
		...
		if err := validateTaskUsage(item.Usage); err != nil {
			return nil, err
		}
		info := relaycommon.TaskInfo{TaskID: item.TaskID, Status: item.Status, Progress: item.Progress, Reason: item.Reason, Url: item.URL, Usage: item.Usage}
		if item.State != nil {
...
// The provider plugin owns all response envelopes, including nested New API
// wrappers; the generic polling shortcut would discard provider usage facts.
func (a *TaskAdaptor) ParseWrappedTaskResult(task *model.Task, resp *http.Response, body []byte) (*relaycommon.TaskInfo, error) {
	return a.ParseTaskResult(task, resp, body)
}

func (a *TaskAdaptor) SanitizeTaskData(body []byte, publicTaskID string) []byte {
	if !a.hasHook(context.Background(), "sanitizeTaskData") {
		return body
	}
	var payload any
	if common.Unmarshal(body, &payload) == nil {
		value, err := a.plugin.Engine.Call(context.Background(), "sanitizeTaskData", payload, publicTaskID)
		if err == nil && value != nil {
	...
	// Do not persist unsanitized provider identities after a sanitizer failure.
	common.SysError("task plugin " + a.plugin.Meta.Key + " failed to sanitize task data")
	return []byte(`{}`)
}
...
func (a *TaskAdaptor) ParseTaskResult(task *model.Task, resp *http.Response, body []byte) (*relaycommon.TaskInfo, error) {
	...
	if err = validateTaskUsage(parsed.Usage); err != nil {
		return nil, err
	}
...
// validateTaskUsage keeps provider display units distinct from billing tokens.
// Provider plugins enforce their tighter limits before this host boundary.
func validateTaskUsage(usage *types.TaskUsage) error {
	if usage == nil {
		return nil
	}
	if usage.Kind != types.TaskUsageKindVideoDuration || usage.Unit != types.TaskUsageUnitSecond {
		return fmt.Errorf("unsupported task usage kind or unit")
	}
	for _, duration := range []float64{usage.Input, usage.Output} {
		if math.IsNaN(duration) || math.IsInf(duration, 0) || duration < 0 || duration > relaycommon.MaxTaskDurationSeconds {
			return fmt.Errorf("invalid task usage duration")
		}
	}
	if math.IsNaN(usage.Total) || math.IsInf(usage.Total, 0) || math.Abs(usage.Total-usage.Input-usage.Output) > 1e-8 {
		return fmt.Errorf("invalid task usage total")
	}
	if usage.InputImages != nil && (*usage.InputImages < 0 || *usage.InputImages > kitdto.MaxImageN) {
		return fmt.Errorf("invalid task usage image count")
	}
	return nil
}
...
func (a *TaskAdaptor) ConvertToOpenAIVideo(task *model.Task) ([]byte, error) {
	...
	if task.Status == model.TaskStatusSuccess {
		artifacts, listErr := a.ListArtifacts(task)
		if listErr != nil {
			return nil, listErr
		}
		for _, artifact := range artifacts {
			if artifact.Key != "video" || artifact.Type != "video" {
				continue
			}
			contentURL, urlErr := service.BuildTaskArtifactContentURL(task.TaskID, artifact.Key)
			if urlErr != nil {
				return nil, fmt.Errorf("build video content URL: %w", urlErr)
			}
			rendered.SetMetadata("url", contentURL)
			break
		}
	}
...
func (a *TaskAdaptor) BuildContentRequest(task *model.Task, artifactKey string, clientRequest channel.TaskArtifactClientRequest) (*channel.TaskContentRequest, error) {
	...
	ctx["channelSettings"] = taskPluginChannelSettings(a.info)
...
func (a *TaskAdaptor) queryContext(task *model.Task, key, baseURL, proxy string) (map[string]any, error) {
	ctx := map[string]any{
		"channelSettings": taskPluginChannelSettings(a.info),
		"taskId":          "",
		"publicTaskId":    "",
		"action":          "",
		"model":           "",
...
// taskPluginChannelSettings exposes only provider transport configuration.
// Never serialize the whole channel settings object: advanced routes can hold
// unrelated credentials and internal channel policy is not a plugin contract.
func taskPluginChannelSettings(info *relaycommon.RelayInfo) map[string]any {
	settings := map[string]any{}
	if info == nil || info.ChannelMeta == nil {
		return settings
	}
	settings["doubao_video_api_mode"] = string(info.ChannelOtherSettings.DoubaoVideoAPIMode)
	settings["doubao_video_submit_path"] = info.ChannelOtherSettings.DoubaoVideoSubmitPath
	settings["doubao_video_fetch_path"] = info.ChannelOtherSettings.DoubaoVideoFetchPath
	return settings
}
...
func (a *TaskAdaptor) submitContext(c *gin.Context, info *relaycommon.RelayInfo) map[string]any {
	...
	ctx["modelMappingResolved"] = a.modelMappingResolved
	if a.requestPrepared && a.submit != nil {
		ctx["preparedRequestBody"] = jsonValue(a.submit.Body)
	}
	ctx["baseUrl"] = info.ChannelBaseUrl
	ctx["channelSettings"] = taskPluginChannelSettings(info)
...
func (a *TaskAdaptor) validateResolvedUsageValue(value any) error {
	...
	case map[string]any:
		for key, item := range typed {
			// Optional request scalars may be explicitly null. Usage facts and
			// final billing multipliers are validated separately and stay strict.
			if item == nil {
				continue
			}
