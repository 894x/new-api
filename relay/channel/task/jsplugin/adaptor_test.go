...
func TestTaskPluginTransportSettingsExcludeUnrelatedChannelSecrets(t *testing.T) {
	source := strings.Replace(mockPlugin, "body: {prompt: ctx.requestBody.prompt}", "body: ctx.channelSettings", 1)
	source = strings.Replace(source, `method: "GET", headers: {"X-Plugin": "query"}`, `method: "POST", body: ctx.channelSettings, headers: {"Content-Type": "application/json"}`, 1)
	plugin, err := pluginruntime.NewRegistry().Register(source, pluginruntime.Options{})
	require.NoError(t, err)
	const want = `{"doubao_video_api_mode":"custom","doubao_video_submit_path":"/submit","doubao_video_fetch_path":"/tasks/{id}"}`
	queryBody := make(chan string, 1)
	...
	info := &relaycommon.RelayInfo{TaskRelayInfo: &relaycommon.TaskRelayInfo{}, ChannelMeta: &relaycommon.ChannelMeta{
		...
		ChannelOtherSettings: dto.ChannelOtherSettings{
			DoubaoVideoAPIMode:    dto.DoubaoVideoAPIModeCustom,
			DoubaoVideoSubmitPath: "/submit", DoubaoVideoFetchPath: "/tasks/{id}",
			AdvancedCustom: &dto.AdvancedCustomConfig{Routes: []dto.AdvancedCustomRoute{{
				IncomingPath: "/private",
				Auth:         &dto.AdvancedCustomRouteAuth{Type: "header", Name: "X-Private-Auth", Value: "unrelated-secret"},
			}}},
		},
	}}
	adaptor := New(plugin)
	...
	c.Set("task_request", map[string]any{"prompt": "fox"})
	reader, err := adaptor.BuildRequestBody(c, info)
	require.NoError(t, err)
	body, err := io.ReadAll(reader)
	require.NoError(t, err)
	assert.JSONEq(t, want, string(body))
	response, err := adaptor.FetchTask(server.URL, "test-key", &model.Task{TaskID: "task-1"}, "")
	require.NoError(t, err)
	require.NoError(t, response.Body.Close())
	assert.JSONEq(t, want, <-queryBody)
}
...
func TestTaskAdaptorPreservesSoraVideoResponseFields(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(string(tc.status), func(t *testing.T) {
			...
			if tc.status == model.TaskStatusSuccess {
				contentURL, err := service.BuildTaskArtifactContentURL(task.TaskID, "video")
				require.NoError(t, err)
				assert.Equal(t, contentURL, fields["url"])
				assert.Equal(t, map[string]any{"url": contentURL}, fields["metadata"])
			} else {
