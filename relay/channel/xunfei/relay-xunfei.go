...
func xunfeiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string, finalBody []byte) (*dto.Usage, *types.NewAPIError) {
	info.StreamStatus = relaycommon.NewStreamStatus()
	domain, authUrl := getXunfeiAuthUrl(c, apiKey, apiSecret, textRequest.Model)
	streamCtx, cancel := context.WithCancel(c.Request.Context())
	stream, err := xunfeiMakeRequest(streamCtx, info, textRequest, domain, authUrl, appId, finalBody)
	if err != nil {
		cancel()
...
func xunfeiHandler(c *gin.Context, info *relaycommon.RelayInfo, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string, finalBody []byte) (*dto.Usage, *types.NewAPIError) {
...
func xunfeiMakeRequest(ctx context.Context, info *relaycommon.RelayInfo, textRequest dto.GeneralOpenAIRequest, domain, authUrl, appId string, finalBody []byte) (*xunfeiResponseStream, error) {
	...
	err = conn.WriteMessage(websocket.TextMessage, finalBody)
	if err != nil {
		_ = conn.Close()
		return nil, err
	}
