...
func TestInitChannelMetaAppliesAdvancedCustomRoutePassThrough(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rootcommon.SetContextKey(ctx, constant.ContextKeyChannelSetting, tt.channelSetting)
			rootcommon.SetContextKey(ctx, constant.ContextKeyChannelOtherSetting, dto.ChannelOtherSettings{AdvancedCustom: advancedCustom})
