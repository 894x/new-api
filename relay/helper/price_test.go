...
func TestModelPriceHelperTieredInputReserveWhenMonthlyDiscountReplacesZeroGroupRatio(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{
		...
		"billing_setting.billing_expr":    `{"monthly-zero-group-tiered":"tier(\"base\", p * 15)"}`,
	...
	priceData, err := ModelPriceHelper(ctx, info, 8192, &types.TokenCountMeta{})
...
func TestModelPriceHelperTieredKeepsBeforeGroupInputReserveForFreeGroupRetry(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{
		...
		"billing_setting.billing_expr":                `{"free-retry-tiered":"tier(\"base\", p * 15)"}`,
...
func TestModelPriceHelperTieredImageReserveIgnoresOutputLimit(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, err)
	// Input only: 100*2 /1e6 * 500000 = 100; output is charged at settlement.
	require.Equal(t, 100, priceData.QuotaToPreConsume)
}
...
func TestModelPriceHelperPreConsumesOriginalWhenMonthlyDiscountIsActive(t *testing.T) {
	...
	ctx.Set("group", "vip")
	info := &relaycommon.RelayInfo{
		OriginModelName: "monthly-discount-preconsume@temperature:0.2",
		UserGroup:       "vip",
		UsingGroup:      "vip",
		StartTime:       time.Unix(10, 0),
	}
	info.BindGroupModelDiscountResolver()
	// Admission freezes all policies before canonical pricing is resolved.
	require.NoError(t, ratio_setting.UpdateModelTieredRatiosByJSONString(`{
		"vip":{"monthly-discount-preconsume":{
			"enabled":true,"effective_from":0,"effective_until":null,"timezone":"UTC",
	...
	require.NoError(t, err)
	require.NotNil(t, info.GroupModelDiscountSnapshot)
	assert.Equal(t, "monthly-discount-preconsume", info.GroupModelDiscountSnapshot.OriginModel)
	assert.Equal(t, 0.9, info.GroupModelDiscountSnapshot.Tiers[0].Ratio)
	require.Equal(t, 20000, priceData.OriginalQuotaToPreConsume)
	require.Equal(t, 20000, priceData.QuotaToPreConsume, "pre-consume must conservatively reserve original quota")

	// Task per-call pricing retains its native model contract; @ modifiers are
	// interpreted by the text relay, not by task plugins.
	info.OriginModelName = "monthly-discount-preconsume"
	priceData, err = ModelPriceHelperPerCall(ctx, info)
	require.NoError(t, err)
	require.NotNil(t, info.GroupModelDiscountSnapshot)
	require.Equal(t, 20000, priceData.OriginalQuota)
...
func (s *priceTestReservation) ReserveForAdmission(quota int) error { return s.Reserve(quota) }
