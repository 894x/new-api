...
func TestGetTaskAdaptorForRequestPinsLegacyMappedPlugin(t *testing.T) {
	...
	require.NotNil(t, adaptor)
	assert.Equal(t, constant.TaskPlatform("sora"), platform)
	// Historical numeric task platforms still resolve to the same driver.
	require.NotNil(t, GetTaskAdaptor(legacyPlatform))
