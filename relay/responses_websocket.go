...
type responsesWSSession struct {
	...
	clientErrorContext *gin.Context
...
func ResponsesWebSocketHelper(c *gin.Context, client *websocket.Conn, runner ResponsesWSRequestRunner) *types.NewAPIError {
	...
	s := &responsesWSSession{ctx: ctx, cancel: cancel, client: client, runner: runner, clientErrorContext: c.Copy(),
...
func (s *responsesWSSession) runRequest(state *responsesWSCallState, message []byte, envelope responsesWSCreateEvent, streamID string, requestID string) {
	...
	if parseErr == nil {
		// Middleware, billing and conversion must read the same normalized body,
		// including clients that nest the model under a response envelope.
		message = create.Body
	}
...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	allowedChannels, _ := common.GetContextKeyType[map[int]struct{}](c, appconstant.ContextKeyAssetAllowedChannelIds)
	common.SetContextKey(c, appconstant.ContextKeySelectionRequestBody, create.Body)
	bodySize := int64(len(create.Body))
	common.SetContextKey(c, appconstant.ContextKeySelectionBodySize, bodySize)
	retry := &service.RetryParam{Ctx: c, TokenGroup: common.GetContextKeyString(c, appconstant.ContextKeyUsingGroup), ModelName: modelName,
		RequestPath: c.Request.URL.Path, RequestBody: create.Body, RequestBodySize: &bodySize, AllowedChannelIds: allowedChannels, Retry: common.GetPointer(0)}
	...
	if s.lockedChannelID != 0 {
		...
		if apiErr = prepareResponsesWSAdmission(c, info, retry); apiErr != nil {
			return apiErr
		}
		var payload []byte
		...
		for ; retry.GetRetry() <= common.RetryTimes; retry.IncreaseRetry() {
			...
			retry.MarkAttempted(channel.Id)
			if info == nil {
				info = relaycommon.GenRelayInfoResponses(c, &create.Request)
				info.IsStream = true
				common.SetContextKey(c, appconstant.ContextKeyIsStream, true)
	...
	for {
		...
		case incoming := <-state.inbox:
			...
			clientIncoming.body = responsesWSDataForClient(c, incoming.body)
			var event struct {
			...
			if err := common.Unmarshal(incoming.body, &event); err != nil {
				...
				if event.Type != "error" && event.StreamID != "" && event.StreamID != create.StreamID {
					if err := s.writeClient(clientIncoming.kind, clientIncoming.body); err != nil {
						s.shutdown()
					}
					continue
				}
				...
				if event.Type == "error" {
					...
					if accepted {
						...
						state.terminal, state.closeAfter = &clientIncoming, ambiguous
						ConsumeResponsesQuota(c, info, accumulator.Finish())
						return nil
					}
					if rejection.Error == nil {
...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	...
	if allowed, constrained := common.GetContextKeyType[map[int]struct{}](c, appconstant.ContextKeyAssetAllowedChannelIds); constrained {
		if _, ok := allowed[channel.Id]; !ok {
			return types.NewErrorWithStatusCode(errors.New("the connection channel cannot serve the referenced assets"), types.ErrorCodeAccessDenied, http.StatusForbidden, types.ErrOptionWithSkipRetry())
		}
	}
	requestBody, _ := common.GetContextKeyType[[]byte](c, appconstant.ContextKeySelectionRequestBody)
	requestSize := int64(len(requestBody))
	if supported, err := channel.SupportsSelectionRequest(model, requestBody, &requestSize); err != nil || !supported {
		if err == nil {
			err = appmodel.ErrParameterCapabilityUnsupported
		}
	...
	if err := service.ValidateSelectedChannelGroupPolicy(c, channel.Id, model); err != nil {
...
func prepareResponsesWSAdmission(c *gin.Context, info *relaycommon.RelayInfo, retry *service.RetryParam) *types.NewAPIError {
	needsTokens, err := service.ConfigureChannelModelCapacity(retry, info)
	if err != nil {
		return types.NewError(err, types.ErrorCodeGetChannelFailed, types.ErrOptionWithSkipRetry())
	}
	if retry.Capacity != nil {
		tokens := info.GetEstimatePromptTokens()
		if needsTokens && !appconstant.CountToken && service.ResolveModelRequestTPMLimit(c) == 0 {
			tokens, err = service.EstimateRequestTokenForCapacity(c, info.Request.GetTokenCountMeta(), info)
			if err != nil {
				return types.NewError(err, types.ErrorCodeCountTokenFailed, types.ErrOptionWithSkipRetry())
			}
		}
		retry.Capacity.PromptTokens = int64(tokens)
	}
	limit := service.ResolveModelRequestTPMLimit(c)
	allowed, _, err := service.ReserveModelRequestTPM(c, info.UserId, limit, info.GetEstimatePromptTokens())
	if err != nil {
		return types.NewError(err, types.ErrorCodeModelRequestTPMCheckFailed, types.ErrOptionWithSkipRetry())
	}
	if !allowed {
		return types.NewErrorWithStatusCode(fmt.Errorf("您已达到 Token 限制：每分钟最多使用 %d tokens", limit), types.ErrorCodeModelRequestTPMLimitExceeded, http.StatusTooManyRequests, types.ErrOptionWithSkipRetry())
	}
	return nil
}
...
func buildResponsesWSCreatePayload(c *gin.Context, info *relaycommon.RelayInfo, req dto.OpenAIResponsesRequest, generate common.RawMessage, streamID string) ([]byte, *types.NewAPIError) {
	...
	if apiErr := admitFinalChannelModelCapacity(c, info, body); apiErr != nil {
		return nil, apiErr
	}
...
// responsesWSDataForClient keeps correlation identifiers when hiding provider
// errors. Accounting consumes the original event before any redaction.
func responsesWSDataForClient(c *gin.Context, body []byte) []byte {
	if c == nil {
		return body
	}
	public, _ := service.StreamErrorDataForClient(c, string(body))
	if public == string(body) {
		return body
	}
	var original, redacted map[string]common.RawMessage
	if common.Unmarshal(body, &original) != nil || common.Unmarshal([]byte(public), &redacted) != nil {
		return []byte(public)
	}
	for _, key := range []string{"event_id", "stream_id", "response_id", "status"} {
		if value, exists := original[key]; exists {
			redacted[key] = value
		}
	}
	result, err := common.Marshal(redacted)
	if err != nil {
		return []byte(public)
	}
	return result
	...
	s.workers.Go(func() {
		for {
			...
			if state := s.getCurrent(); state != nil {
				...
				case <-state.done:
					if err == nil {
						if writeErr := s.writeClient(kind, responsesWSDataForClient(s.clientErrorContext, body)); writeErr != nil {
							s.shutdown()
							return
						}
					} else {
