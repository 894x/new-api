...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/model-docs", middleware.HeaderNavModuleAuth("model_docs"), controller.GetModelDocsCatalog)
		apiRouter.GET("/model-docs/:slug", middleware.HeaderNavModuleAuth("model_docs"), controller.GetModelDoc)
		apiRouter.GET("/token-cost-pricing", middleware.HeaderNavModuleAuth("token_cost"), controller.GetPricing)
		...
		perfAnalyticsRoute := apiRouter.Group("/perf-analytics")
		{
			...
			perfAnalyticsRoute.GET("/admin/options", middleware.AdminAuth(), controller.GetPerfAnalyticsAdminOptions)
		}
		...
		apiRouter.POST("/asset-library/upload", middleware.TokenOrUserAuth(), controller.UploadAssetLibraryFile)
		apiRouter.GET("/asset-library/storage", middleware.TokenOrUserAuth(), controller.GetAssetStorageUsage)
		apiRouter.GET("/asset-library/assets/:id/content", middleware.TokenOrUserAuth(), controller.GetAssetLibraryContent)
		apiRouter.HEAD("/asset-library/assets/:id/content", middleware.TokenOrUserAuth(), controller.GetAssetLibraryContent)
		apiRouter.GET("/asset-library/assets/:id/preview", middleware.UserAuth(), controller.GetAssetLibraryImagePreview)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.GET("/wechatpay/status/:trade_no", middleware.DisableCache(), controller.GetWechatPayStatus)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.PUT("/group-pricing", controller.UpdateGroupPricingOptions)
			optionRoute.PUT("/dynamic_routing", controller.UpdateDynamicRoutingOptions)
		...
		registerAssetLibraryAdminRoutes(apiRouter)
		...
		logRoute.GET("/token", middleware.CORS(), middleware.TokenAuthReadOnly(), middleware.LogQueryRateLimit(), controller.GetLogByKey)
		...
		modelsRoute.Use(middleware.AdminAuth())
		{
			...
			modelsRoute.GET("/:id/documents", controller.GetModelDocumentsEditor)
			modelsRoute.PUT("/:id/documents/:interface_key", controller.SaveModelDocumentVariantDraft)
			modelsRoute.POST("/:id/documents/:interface_key/publish", controller.PublishModelDocumentVariant)
			modelsRoute.POST("/:id/documents/preview", controller.PreviewModelDocumentEditor)
			modelsRoute.DELETE("/:id/documents/:interface_key", controller.DeleteModelDocumentVariant)
