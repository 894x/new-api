...
func taskPluginProtocolHandlers(protocol, operation string) ([]gin.HandlerFunc, error) {
	...
	case "openai_responses.create":
		return []gin.HandlerFunc{
			...
			middleware.ModelRequestRateLimit(), middleware.PinTaskPluginEndpoint(), middleware.TaskPluginEndpointOnly(middleware.AssetLibraryRouting()), middleware.PrepareTaskPluginEndpoint(), middleware.Distribute(),
			func(c *gin.Context) {
	...
	case "openai_video.create":
		return []gin.HandlerFunc{
			...
			middleware.PinTaskPluginEndpoint(), middleware.TaskPluginEndpointOnly(middleware.ModelRequestRateLimit()), middleware.AssetLibraryRouting(), middleware.PrepareTaskPluginEndpoint(), middleware.Distribute(),
