...
func TestVideoContentRoutePreservesDashboardAndTokenAuthorization(t *testing.T) {
	...
	require.NoError(t, model.DB.Where("username = ?", "asset-e2e-other").First(&other).Error)
	otherSession, err := service.CreateLoginSession(other.Id, "password", "127.0.0.1", "video-content-test")
	require.NoError(t, err)
	for _, owner := range []model.User{f.user, other} {
		task := model.Task{TaskID: model.GenerateTaskID(), UserId: owner.Id, Platform: "54", Status: model.TaskStatusSuccess,
			PrivateData: model.TaskPrivateData{ResultURL: "data:video/mp4;base64,ZGF0YQ=="},
		}
		...
		for _, tc := range []struct {
			...
			{"dashboard administrator", f.adminToken, true},
			{"administrator API token", "assete2euserkey", owner.Id == f.user.Id},
			{"dashboard common user", otherSession.AccessToken, owner.Id == other.Id},
			{"common API token", "assete2eotherkey", owner.Id == other.Id},
		} {
			t.Run(tc.name+"/"+owner.Username, func(t *testing.T) {
				status, headers, body := f.request(t, http.MethodGet, "/v1/videos/"+task.TaskID+"/content", tc.credential, nil)
				if tc.permitted {
					assert.Equal(t, http.StatusOK, status, string(body))
					assert.Equal(t, "data", string(body))
					assert.Contains(t, headers.Get("Content-Type"), "video/mp4")
				} else {
		...
		status, _, _ := f.request(t, http.MethodGet, "/v1/videos/"+task.TaskID+"/content", "", nil)
