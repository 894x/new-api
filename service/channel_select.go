...
func SelectChannelForRequest(c *gin.Context, modelName string, retry *RetryParam) (*model.Channel, string, *ChannelSelectError) {
	...
	if pin, found, overridden := constraints.ResolvedPin(); found {
		...
		parametersSupported, capabilityErr := channel.SupportsSelectionRequest(modelName, retry.RequestBody, retry.RequestBodySize)
		if capabilityErr != nil || !parametersSupported {
			if capabilityErr == nil {
				capabilityErr = model.ErrParameterCapabilityUnsupported
			}
			return nil, "", &ChannelSelectError{StatusCode: http.StatusBadRequest, MessageID: i18n.MsgDistributorInvalidRequest, Params: map[string]any{"Error": capabilityErr.Error()}}
		}
		if retry.AllowedChannelIds != nil {
			if _, allowed := retry.AllowedChannelIds[channel.Id]; !allowed {
				return nil, "", &ChannelSelectError{StatusCode: http.StatusServiceUnavailable, Message: "The specified channel has no replica for every referenced asset"}
			}
		}
		return channel, "", nil
	}
	...
	if retry.GetRetry() == 0 {
		...
		if preferredChannelID, found := GetPreferredChannelByAffinity(c, modelName, usingGroup); found &&
			...
			if affinitySatisfied {
				parametersSupported, capabilityErr := preferred.SupportsSelectionRequest(modelName, retry.RequestBody, retry.RequestBodySize)
				affinitySatisfied = capabilityErr == nil && parametersSupported && ValidateSelectedChannelGroupPolicy(c, preferred.Id, modelName) == nil
				if retry.AllowedChannelIds != nil {
					_, allowed := retry.AllowedChannelIds[preferred.Id]
					affinitySatisfied = affinitySatisfied && allowed
				}
			}
			if affinitySatisfied {
				if usingGroup == "auto" {
	...
	if channel == nil {
		...
		if err != nil {
			if errors.Is(err, model.ErrParameterCapabilityUnsupported) {
				return nil, selectGroup, &ChannelSelectError{StatusCode: http.StatusBadRequest, MessageID: i18n.MsgDistributorInvalidRequest, Params: map[string]any{"Error": err.Error()}}
			}
