...
func PrepareImageBillingForRequest(c *gin.Context, info *relaycommon.RelayInfo, count int) *types.NewAPIError {
	...
	if snap := info.TieredBillingSnapshot; snap != nil && snap.BillingMode == "tiered_expr" {
		...
		if err == nil {
			originalQuota, err = billingexpr.QuotaRoundStrict(beforeGroup)
		}
		if err == nil {
		...
		if err == nil {
			originalQuota, err = common.QuotaFromFloatStrict(beforeGroup)
		}
	}
	if err != nil {
	...
	if info.GroupModelDiscountSnapshot != nil {
		if beforeGroup > 0 && originalQuota == 0 {
			originalQuota = 1
		}
		// Monthly discount settlement consumes the pre-group amount. Reserve
		// that amount before submission, just like initial admission does.
		quota = originalQuota
	}
	info.PriceData.OriginalQuotaToPreConsume = originalQuota
