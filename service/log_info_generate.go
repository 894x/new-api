...
func AppendUpstreamResponseAdminInfo(ctx *gin.Context, other *model.LogOther) {
	if ctx == nil || other == nil {
		return
	}
	if upstreamResponseId := ctx.GetString(common.UpstreamResponseIdKey); upstreamResponseId != "" {
		other.SetAdmin("upstream_response_id", upstreamResponseId)
	}
	if headers, ok := common.GetContextKeyType[map[string]string](ctx, common.UpstreamResponseHeadersKey); ok && len(headers) > 0 {
		...
		other.SetAdmin("upstream_request_ids", copiedHeaders)
	}
}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	other.SetPublic("quota_per_unit", common.QuotaPerUnit)
	...
	AppendUpstreamResponseAdminInfo(ctx, other)
	if relayInfo.RequestTiming != nil {
		relayInfo.RequestTiming.Mark(common.RequestTimingCompleted, time.Now())
		other.SetAdmin("request_timing", relayInfo.RequestTiming.Snapshot())
	...
	appendParameterCapabilityInfo(relayInfo, other)
	appendStreamStatus(relayInfo, other)
	return other
}

func appendParameterCapabilityInfo(relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	if relayInfo == nil || other == nil || len(relayInfo.ParameterCapabilityAudit) == 0 {
		return
	}
	other.SetAdmin("parameter_capability", relayInfo.ParameterCapabilityAudit)
}
