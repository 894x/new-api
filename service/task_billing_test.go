...
func TestTaskTokenSettlementAuditsInputSaturation(t *testing.T) {
	truncate(t)
	const userID, channelID = 15, 15
	seedUser(t, userID, 1_000_000)
	seedChannel(t, channelID)
	seedChargedAccounting(t, userID, channelID, 0, 100, 1)
	task := makeTask(userID, channelID, 100, 0, BillingSourceWallet, 0)
	task.PrivateData.BillingContext.ModelRatio = 0.000001
	task.PrivateData.BillingContext.GroupRatio = 1
	require.NoError(t, model.DB.Create(task).Error)
	tokens, clamp := common.QuotaRoundChecked(1e30)
	require.NotNil(t, clamp)
	require.True(t, RecalculateTaskQuotaByTokens(context.Background(), task, tokens, clamp))
	assert.Equal(t, 2147, task.Quota)
	assert.Equal(t, 1_000_000-(2147-100), getUserQuota(t, userID))
	var log model.Log
	require.NoError(t, model.LOG_DB.Where("user_id = ? AND type = ?", userID, model.LogTypeConsume).Last(&log).Error)
	var other map[string]any
	require.NoError(t, common.UnmarshalJsonStr(log.Other, &other))
	...
	assert.Equal(t, float64(common.MaxQuota), saturation["clamped"])
...
func TestSettle_TaskRequestProbesRemainPrivateAndTraceActualCharge(t *testing.T) {
	truncate(t)
	seedUser(t, 36, 10000)
	expression := `tier("base", u("seconds") * u("clips")) * (param("quality") == "high" ? 2 : 1) * (header("X-Plan") == "premium" ? 3 : 1)`
	frozen, err := billingexpr.SnapshotRequestInput(expression, billingexpr.RequestInput{Body: []byte(`{"quality":"high","prompt":"private prompt"}`), Headers: map[string]string{"X-Plan": "premium", "Authorization": "private token"}}, time.Time{})
	require.NoError(t, err)
	task := makeTask(36, 0, 60, 0, BillingSourceWallet, 0)
	task.PrivateData.BillingContext.TieredSnapshot = &billingexpr.BillingSnapshot{ExprString: expression, ExprHash: billingexpr.ExprHashString(expression), GroupRatio: 1, QuotaPerUnit: 1, TaskUsageBilling: true, UsageFacts: map[string]any{"seconds": 5.0, "clips": 2.0}, RequestInput: frozen}
	require.True(t, settleTaskBillingOnComplete(context.Background(), &mockAdaptor{}, task, &relaycommon.TaskInfo{Status: model.TaskStatusSuccess, UsageFacts: map[string]any{"seconds": 10.0}}))
	assert.Equal(t, 120, task.Quota)
	assert.Equal(t, map[string]any{"seconds": 10.0, "clips": 2.0}, task.PrivateData.BillingContext.TieredSnapshot.UsageFacts)
	log := getLastLog(t)
	require.NotNil(t, log)
	var other map[string]any
	require.NoError(t, common.UnmarshalJsonStr(log.Other, &other))
	rules, ok := other["request_rules"].([]any)
	require.True(t, ok)
	require.Len(t, rules, 2)
	for _, rule := range rules {
		assert.Equal(t, true, rule.(map[string]any)["matched"])
	}
	for _, secret := range []string{"private prompt", "private token", "request_input"} {
