...
type wrappedTaskResultParser interface {
	ParseWrappedTaskResult(task *model.Task, resp *http.Response, body []byte) (*relaycommon.TaskInfo, error)
}
...
func updateBatchTasks(ctx context.Context, adaptor BatchTaskPollingAdaptor, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	for upstreamID, responseItem := range responseItems {
		...
		if responseItem.TaskInfo.Usage != nil {
			task.PrivateData.Usage = responseItem.TaskInfo.Usage
			task.Usage = responseItem.TaskInfo.Usage
		}
...
func updateVideoSingleTask(ctx context.Context, adaptor TaskPollingAdaptor, ch *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	if wrappedAdaptor, ok := adaptor.(wrappedTaskResultParser); ok {
		if taskResult, err = wrappedAdaptor.ParseWrappedTaskResult(task, resp, responseBody); err != nil {
	...
	if privateID := task.PrivateData.UpstreamTaskID; privateID != "" && privateID != task.TaskID {
		var payload any
		if common.Unmarshal(storedResponseBody, &payload) == nil {
			if sanitized, marshalErr := common.Marshal(replacePrivateTaskID(payload, privateID, task.TaskID)); marshalErr == nil {
				storedResponseBody = sanitized
			}
		}
	}
...
func finalizeTerminalTask(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	if task.Status == model.TaskStatusFailure && !billingSettled && taskNeedsBillingRefund(task) {
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	if bc := task.PrivateData.BillingContext; bc != nil && bc.TieredSnapshot != nil {
		...
		if bc.DiscountSettlementID != "" {
			originalQuota, originalClamp := common.QuotaRoundChecked(result.ActualQuotaBeforeGroup)
			if result.ActualQuotaBeforeGroup > 0 && originalQuota == 0 {
				originalQuota = 1
			}
			adjustTaskMonthlyModelCharge(ctx, task, originalQuota, result.ActualQuotaAfterGroup, "任务用量表达式结算", originalClamp, result.Clamp)
		} else {
