...
func (a *opaqueDataPollingAdaptor) FetchTask(_ string, _ string, task *model.Task, _ string) (*http.Response, error) {
...
func (a *opaqueDataPollingAdaptor) ParseTaskResult(*model.Task, *http.Response, []byte) (*relaycommon.TaskInfo, error) {
	a.parseCalls++
...
func (a *wrappedTaskPollingAdaptor) FetchTask(_ string, _ string, task *model.Task, _ string) (*http.Response, error) {
	taskID := task.GetUpstreamTaskID()
...
func (a *wrappedTaskPollingAdaptor) ParseTaskResult(*model.Task, *http.Response, []byte) (*relaycommon.TaskInfo, error) {
	return nil, assert.AnError
}

func (a *wrappedTaskPollingAdaptor) ParseWrappedTaskResult(*model.Task, *http.Response, []byte) (*relaycommon.TaskInfo, error) {
	return &relaycommon.TaskInfo{
...
func TestDispatchPlatformUpdateUsesFetchMode(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", channelID).Update("settings", `{"doubao_video_api_mode":"custom","doubao_video_fetch_path":"/tasks/{id}"}`).Error)
	...
	assert.Equal(t, dto.DoubaoVideoAPIModeCustom, batch.initializedSettings.DoubaoVideoAPIMode)
	assert.Equal(t, "/tasks/{id}", batch.initializedSettings.DoubaoVideoFetchPath)
