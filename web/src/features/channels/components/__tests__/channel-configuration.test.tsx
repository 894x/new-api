  ...
  await waitFor(() =>
    expect(post).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        channel: expect.objectContaining({ type: 105 }),
      }),
      expect.anything()
    )
  )
...
test('an operator without sensitive write permission can discover saved models and update routing', async () => {
  ...
  fireEvent.change(screen.getByLabelText('RPM'), { target: { value: '60' } })
  fireEvent.change(screen.getByLabelText('TPM'), { target: { value: '12000' } })
  await user.click(screen.getByRole('button', { name: 'Update Channel' }))
  await waitFor(() => expect(put).toHaveBeenCalled())
  expect(put.mock.calls[0]?.[1]).toMatchObject({
    id: 42,
