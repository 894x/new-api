...
export function ChannelMutateDrawer({
  ...
  const doubaoVideoAPIMode = formValues.doubao_video_api_mode
  const currentParamOverride = formValues.param_override
  const currentParameterCapabilities = formValues.parameter_capabilities
  const {
      ...
      <div className='grid gap-4 sm:grid-cols-2'>
        ...
        {(['rpm', 'tpm'] as const).map((name) => (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{name === 'rpm' ? t('RPM') : t('TPM')}</FormLabel>
                <FormControl>
                  <Input
                    type='number'
                    min={0}
      ...
      {currentType === 54 && (
        ...
        >
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Upstream task API')}</FormLabel>
                <Select
                  items={[
                    {
                      value: 'v3',
                ...
                >
                  ...
                  <SelectContent alignItemWithTrigger={false}>
                    <SelectGroup>
                      <SelectItem value='v3'>{t('Doubao V3')}</SelectItem>
                ...
                <FormDescription>
                  {t('Choose a built-in API format or configure custom paths')}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          {doubaoVideoAPIMode === 'custom' && (
            <>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('Submit task path')}</FormLabel>
                    <FormControl>
                      <Input placeholder='/v1/video/generations' {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('Query task path')}</FormLabel>
                    <FormControl>
                      <Input
                    ...
                    <FormDescription>
                      {t('Query task path must include {id}')}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <>
            ...
            {isEditing && channelId && (
              <ModelRoutingOverridesEditor
                channelId={channelId}
                disabled={Boolean(form.formState.dirtyFields.models)}
                disabledReason={t(
                  ...
                  <FormItem className='space-y-3'>
                    <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
                      ...
                      <Button
                        ...
                        variant='outline'
                        size='sm'
                        onClick={() => setParameterCapabilityEditorOpen(true)}
                        disabled={sensitiveLocked || isSubmitting}
                      >
                        ...
                        <>
                          <Badge variant='secondary'>
                            {t('{{count}} channel default(s)', {
                              count: parameterCapabilityStats.defaultCount,
                            })}
                          </Badge>
                          <Badge variant='outline'>
                            {t('{{count}} model rule(s)', {
                              count: parameterCapabilityStats.ruleCount,
                            })}
                          </Badge>
                          <Badge variant='outline'>
                            {t('{{count}} parameter override(s)', {
                              count: parameterCapabilityStats.overrideCount,
                            })}
                          </Badge>
                        </>
                      ) : (
