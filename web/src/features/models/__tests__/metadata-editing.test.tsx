...
beforeEach(() => {
  usePricingPreferencesStore.getState().setCurrency('USD')
  useSystemConfigStore
    .getState()
...
describe('model pricing entry', () => {
  it('saves pricing and opens connections for a channel model without creating metadata', async () => {
    ...
    const get = vi.spyOn(api, 'get').mockImplementation(async (url) => {
      if (url === '/api/channel/model-routing-overrides') {
    ...
    expect(
      screen.getByRole('button', { name: 'Edit model document' })
    ).toBeDisabled()
    expect(
...
describe('metadata editing', () => {
  ...
  it('allows an administrator to save metadata without loading or changing system pricing', async () => {
    ...
    await user.click(screen.getByRole('switch', { name: 'Model document' }))
    await user.click(
  ...
  it.each([
    { currency: 'USD' as const, rate: 1, displayed: '1.5', reloaded: '2' },
    {
    ...
    'keeps metadata drafts and saves $currency pricing independently across tabs',
    async ({ currency, rate, displayed, reloaded }) => {
      usePricingPreferencesStore
        .getState()
        .setCurrency(currency === 'USD' ? 'USD' : 'site')
      ...
      const get = vi.spyOn(api, 'get').mockImplementation(async (url) => {
        if (url === '/api/status') {
          return {
            data: {
              success: true,
              data: { quota_display_type: currency, usd_exchange_rate: rate },
            },
          }
        }
        if (url === '/api/models/7') {
      ...
      await waitFor(() => expect(price).toHaveValue(displayed))
      await user.clear(price)
      await user.type(price, '0')
      ...
      await waitFor(() =>
        expect(
          screen.getByRole('textbox', { name: 'Fixed price' })
        ).toHaveValue(reloaded)
      )
      await user.clear(screen.getByRole('textbox', { name: 'Fixed price' }))
      await user.type(screen.getByRole('textbox', { name: 'Fixed price' }), '0')
      await user.click(
        screen.getByRole('button', { name: 'Save model prices' })
      )
      await waitFor(() =>
