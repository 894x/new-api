...
it('requires saved metadata before toggling a channel-only model document', async () => {
  const put = vi.spyOn(api, 'put')
  await renderList([{ ...channel, doc_available: true, doc_enabled: 0 }])
  const control = screen.getByRole('switch', { name: 'Enable model document' })
