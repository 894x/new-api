...
import { updateGroupPricingOptions, updateSystemOption } from '../../api'
...
vi.mock('../../api', () => ({
  ...
  updateGroupPricingOptions: vi.fn(),
...
describe('ratio settings card tiered-discount persistence', () => {
  ...
  test('atomically replaces referenced pricing groups and tiered policies', async () => {
    vi.mocked(updateGroupPricingOptions).mockResolvedValue({
      success: true,
      message: '',
    })
    ...
    fireEvent.input(
      screen.getByRole('textbox', { name: 'Monthly tiered discounts' }),
      {
        target: {
          value:
            '{"replacement":{"gpt-5":{"enabled":false,"effective_from":1788192000,"effective_until":null,"timezone":"UTC","tiers":[{"min_monthly_original_quota":0,"ratio":0.8}]}}}',
        },
      }
    )
    ...
    const request = vi.mocked(updateGroupPricingOptions).mock.calls[0]?.[0]
    expect(JSON.parse(request?.group_ratio ?? '{}')).toEqual({
      replacement: 1.2,
    })
    expect(JSON.parse(request?.model_tiered_ratios ?? '{}')).toEqual({
      replacement: {
        progress_basis: 'original',
        models: {
          'gpt-5': {
  ...
  test('disables group save while the atomic pricing update is pending', async () => {
    let finishRequest:
      | ((value: { success: boolean; message: string }) => void)
      | undefined
    vi.mocked(updateGroupPricingOptions).mockImplementation(
      () =>
        new Promise((resolve) => {
    ...
    await waitFor(() =>
      expect(updateGroupPricingOptions).toHaveBeenCalledOnce()
    )
    expect(save).toBeDisabled()
    finishRequest?.({ success: true, message: '' })
  })
  ...
  test('keeps partial-failure drafts and invalidates options only after a complete retry', async () => {
    ...
    ).toEqual(['TopupGroupRatio', 'TopupGroupRatio'])
    expect(invalidateQueries).toHaveBeenCalledTimes(1)
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['system-options'],
    })
    const pricingRequest = vi.mocked(updateGroupPricingOptions).mock
      .calls[0]?.[0]
    expect(JSON.parse(pricingRequest?.model_tiered_ratios ?? '{}')).toEqual({
      premium: {
        progress_basis: 'original',
        models: {
          'gpt-5': {
