...
it.each([true, false])(
  'renders asset timing only when the server includes admin metadata: %s',
  async (adminView) => {
    const log: AuditLog = {
      ...entry,
      ...
      other: {
        op: {
          action: 'asset_library.asset.create',
          params: { id: 'asset-1', name: 'portrait', asset_type: 'Image' },
        },
        ...(adminView
          ? {
              admin_info: {
    ...
    if (adminView) {
      expect(within(dialog).getByText('Request Timeline')).toBeVisible()
      expect(
        within(dialog).getAllByText('Media Inspection').length
      ).toBeGreaterThan(0)
    } else {
      expect(
        within(dialog).queryByText('Request Timeline')
