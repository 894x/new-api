...
import { parseLogOther, renderAuditContent } from '../../lib/format'
...
export function buildAuditDetails(entry: AuditLog, t: TFunction) {
  ...
  const timing = parseLogOther(JSON.stringify(metadata))?.admin_info
    ?.asset_timing
  const assetTiming = timing?.version === 1 ? timing : undefined
  if (assetTiming) delete adminExtra.asset_timing
  for (const key of [
    'admin_id',
    'admin_username',
    'admin_role',
