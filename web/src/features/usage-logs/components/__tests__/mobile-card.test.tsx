...
it('keeps video input and output usage when async jobs have no token counts', () => {
  ...
  expect(screen.getByText('Input').parentElement).toHaveTextContent('Input 8s')
  expect(screen.getByText('Output').parentElement).toHaveTextContent(
    'Output 7.5s'
  )
  expect(screen.getByText('Total 15.5s')).toBeVisible()
})

it('keeps generation throughput separate from overall throughput on mobile', () => {
  renderLogs({
    logs: [
      {
        ...log,
        ...
        other: JSON.stringify({ frt: 500 }),
      },
    ],
  })
  expect(screen.getByText('50 t/s')).toBeVisible()
  expect(screen.getByText('Generation 66 t/s')).toBeVisible()
})
