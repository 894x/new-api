...
export function TaskDetailsDialog(props: TaskDetailsDialogProps) {
  ...
  const properties =
    typeof props.log.properties === 'object' ? props.log.properties : undefined
  let rawData = ''
  if (props.open && props.canViewRawData && props.log.data != null) {
    rawData =
      typeof props.log.data === 'string'
        ? tryPrettyJson(props.log.data)
      ...
      <div className='space-y-3'>
        ...
        {rawData && (
          <DetailSection label={t('Raw JSON')}>
            <CopyButton value={rawData} aria-label={t('Copy')} />
            <pre
              aria-label={t('Raw JSON')}
              className='max-h-[60vh] overflow-auto font-mono text-xs break-all whitespace-pre-wrap'
            >
              {rawData}
            </pre>
          </DetailSection>
